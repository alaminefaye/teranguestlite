...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-title-md font-semibold text-gray-800 dark:text-white/90">Gestion des Réservations</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Gérez toutes les réservations</p>
        </div>
        <div class="flex items-center gap-3">
            <div class="flex items-center gap-2">
                <a href="{{ route('reservations.index', array_merge(request()->query(), ['export' => 'excel'])) }}" 
                ...
                <a href="{{ route('reservations.index', array_merge(request()->query(), ['export' => 'pdf'])) }}" 
                   class="inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        <form method="GET" action="{{ route('reservations.index') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Paiement</label>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-800">
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-400">Siège(s)</th>
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-400">Créé par</th>
                ...
                <tbody>
                    @forelse($bookings as $booking)
                        <tr class="border-b border-gray-100 dark:border-gray-800 table-row-hover">
                            <td class="px-4 py-3 font-medium text-gray-800 dark:text-white/90">{{ $booking->booking_reference }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ $booking->user->name }}</td>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                @if($booking->seats && $booking->seats->count() > 0)
                                    ...
                                    <span class="text-xs text-gray-500 dark:text-gray-400">({{ $booking->seats->count() }})</span>
                                @else
                                    {{ $booking->seat_number }}
                                @endif
                            </td>
                            <td class="px-4 py-3 font-medium text-gray-800 dark:text-white/90">{{ number_format($booking->total_price, 0, ',', ' ') }} FCFA</td>
                            <td class="px-4 py-3">
                                @php
                                    $statusColors = [
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $statusColors[$booking->status] ?? 'bg-gray-50 text-gray-600' }}">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $paymentColors[$booking->payment_status] ?? 'bg-gray-50 text-gray-600' }}">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                @if($booking->createdBy)
                                    <div class="flex flex-col">
                                        <span class="font-medium">{{ $booking->createdBy->name }}</span>
                                ...
                                @else
                                    <span class="text-gray-400 dark:text-gray-500">Application mobile</span>
                                @endif
                            </td>
