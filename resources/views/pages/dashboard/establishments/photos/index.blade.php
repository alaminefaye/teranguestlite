...
<div class="mb-6">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Galerie : {{ $establishment->name }}</h1>
        <a href="{{ route('dashboard.establishments.show', $establishment) }}" class="text-sm text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200">← Retour à la fiche</a>
    </div>
</div>
...
<div class="mb-8 rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    ...
    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Taille max : 20 Mo. Formats : JPG, PNG.</p>
    <form action="{{ route('dashboard.establishments.photos.store', $establishment) }}" method="POST" enctype="multipart/form-data" class="flex flex-wrap items-end gap-4">
        @csrf
        ...
        <div class="min-w-[180px]">
            <label for="caption" class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Légende (optionnel)</label>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-6">
        @forelse($photos as $photo)
            <div class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-2">
                    @if($photo->caption)<p class="text-sm font-medium text-gray-800 dark:text-white/90 truncate">{{ $photo->caption }}</p>@endif
                    <form action="{{ route('dashboard.establishments.photos.destroy', [$establishment, $photo]) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer cette photo ?');">
                        @csrf
                        @method('DELETE')
        ...
        @empty
            <div class="col-span-full text-center py-12 text-gray-500 dark:text-gray-400">
                Aucune photo. Ajoutez-en avec le formulaire ci-dessus.
            </div>
        @endforelse
    </div>
</div>
