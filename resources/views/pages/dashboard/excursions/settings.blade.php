...
<form action="{{ route('dashboard.excursions.settings.update') }}" method="POST" enctype="multipart/form-data">
    ...
    <div id="document-section"
         ...
         style="{{ ($excursionsSettings['display_mode'] ?? 'catalog') === 'document' ? '' : 'display:none' }}">
        ...
        @if(!empty($excursionsSettings['document_url']))
            <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <a href="{{ $excursionsSettings['document_url'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">
                        {{ basename($excursionsSettings['document_path'] ?? $excursionsSettings['document_url']) }}
                    </a>
                </div>
                <a href="{{ $excursionsSettings['document_url'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
            </div>
        @endif
        <div class="flex items-center justify-center w-full">
            <label for="document-upload" class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p class="mb-1 text-sm text-gray-500 dark:text-gray-400"><span class="font-semibold text-brand-600 dark:text-brand-400">Cliquez pour choisir</span> ou glissez-déposez</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500">PDF, JPG, PNG, WEBP — max 20 Mo</p>
                </div>
                <input id="document-upload" type="file" name="document" class="hidden" accept=".pdf,.jpg,.jpeg,.png,.webp" onchange="updateFileName(this)">
            </label>
        </div>
