...
<div class="mb-6 flex items-center gap-4">
    <a href="{{ route('dashboard.excursions.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-sm flex items-center gap-1">
        ← Retour aux excursions
    </a>
</div>

<div class="mb-6">
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">⚙️ Paramètres d'affichage — Excursions</h1>
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Choisissez comment la section Excursions s'affiche dans l'application mobile.</p>
</div>
...
<form action="{{ route('dashboard.excursions.settings.update') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mb-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label class="relative flex cursor-pointer rounded-xl border-2 p-5 transition-all
                {{ ($excursionsSettings['display_mode'] ?? 'catalog') === 'catalog' ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-brand-300' }}">
                <input type="radio" name="display_mode" value="catalog" class="sr-only"
                    {{ ($excursionsSettings['display_mode'] ?? 'catalog') === 'catalog' ? 'checked' : '' }}
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <p class="font-semibold text-gray-800 dark:text-white/90">Catalogue d'excursions</p>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Affiche la liste des excursions avec détails et réservation.</p>
                    </div>
                </div>
            </label>
            <label class="relative flex cursor-pointer rounded-xl border-2 p-5 transition-all
                {{ ($excursionsSettings['display_mode'] ?? 'catalog') === 'document' ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-brand-300' }}">
                <input type="radio" name="display_mode" value="document" class="sr-only"
                    {{ ($excursionsSettings['display_mode'] ?? 'catalog') === 'document' ? 'checked' : '' }}
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Ouvre directement un document dans l'app (brochure, programme, carte…).</p>
                    </div>
                </div>
            </label>
        </div>
    ...
    <div id="document-section"
         class="mb-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 {{ ($excursionsSettings['display_mode'] ?? 'catalog') === 'document' ? '' : 'hidden' }}">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                @if(!empty($excursionsSettings['document_url_fr']))
                    <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <a href="{{ $excursionsSettings['document_url_fr'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">{{ basename($excursionsSettings['document_path_fr'] ?? $excursionsSettings['document_url_fr']) }}</a>
                        </div>
                        <a href="{{ $excursionsSettings['document_url_fr'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
                    </div>
                @endif
            ...
            <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                @if(!empty($excursionsSettings['document_url_en']))
                    <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                        <span class="text-success-600 dark:text-success-400 text-xl">✅</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-success-700 dark:text-success-300">Version EN actuelle</p>
                            <a href="{{ $excursionsSettings['document_url_en'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">{{ basename($excursionsSettings['document_path_en'] ?? $excursionsSettings['document_url_en']) }}</a>
                        </div>
                        <a href="{{ $excursionsSettings['document_url_en'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
                    </div>
                @endif
        ...
        @if(!empty($excursionsSettings['legacy_document_url']))
            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                ...
                <a href="{{ $excursionsSettings['legacy_document_url'] }}" target="_blank" class="underline">{{ basename($excursionsSettings['legacy_document_path'] ?? $excursionsSettings['legacy_document_url']) }}</a>
            </div>
        @endif
    ...
    <div class="flex justify-end gap-3">
        <a href="{{ route('dashboard.excursions.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">Annuler</a>
