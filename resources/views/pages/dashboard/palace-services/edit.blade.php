...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
        ...
        <span>/</span>
        <a href="{{ route('dashboard.palace-services.show', $service) }}" class="hover:text-brand-500">{{ $service->name }}</a>
        <span>/</span>
        <span>Modifier</span>
    </div>
...
<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.palace-services.update', $service) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="category" id="category" required class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
                    <option value="concierge" {{ old('category', $service->category) == 'concierge' ? 'selected' : '' }}>Conciergerie</option>
                    <option value="transport" {{ old('category', $service->category) == 'transport' ? 'selected' : '' }}>Transport</option>
                    <option value="vip" {{ old('category', $service->category) == 'vip' ? 'selected' : '' }}>VIP</option>
                    <option value="butler" {{ old('category', $service->category) == 'butler' ? 'selected' : '' }}>Butler</option>
                </select>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="price_on_request" id="price_on_request" value="1" {{ old('price_on_request', $service->price_on_request) ? 'checked' : '' }}
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_premium" id="is_premium" value="1" {{ old('is_premium', $service->is_premium) ? 'checked' : '' }}
                    class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500 dark:border-gray-700">
        ...
        <div class="mt-8 flex items-center gap-4">
            ...
            <a href="{{ route('dashboard.palace-services.show', $service) }}" class="inline-flex items-center px-6 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800">Annuler</a>
        </div>
    </form>
</div>
@endsection
