...
<div class="mb-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('dashboard.palace-services.edit', $service) }}" class="inline-flex items-center px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600 dark:bg-brand-600 dark:hover:bg-brand-700 text-sm">Modifier</a>
            <form action="{{ route('dashboard.palace-services.destroy', $service) }}" method="POST" onsubmit="return confirm('Supprimer ce service ?');" class="inline">
                @csrf
                @method('DELETE')
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="md:col-span-2 rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        @if($service->image)
            <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->name }}" class="w-full max-h-64 object-contain rounded-lg border border-gray-200 dark:border-gray-700 mb-4">
        @else
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <p class="text-2xl font-bold text-brand-600 dark:text-brand-400">{{ $service->formatted_price }}</p>
        ...
        @if($service->is_premium)
            <span class="inline-flex items-center gap-1 text-sm font-medium text-brand-600 dark:text-brand-400 mt-2">★ Premium</span>
        @endif
    </div>
</div>

<div class="mt-6">
    <a href="{{ route('dashboard.palace-services.index') }}" class="inline-flex items-center px-6 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800">← Retour à la liste</a>
</div>
@endsection
