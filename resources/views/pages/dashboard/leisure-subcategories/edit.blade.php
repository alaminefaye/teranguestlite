...
<div class="mb-6">
    ...
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Modifier l'activité</h1>
</div>

<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.leisure-categories.subcategories.update', [$parent, $subcategory]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="2"
                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">{{ old('description', $subcategory->description) }}</textarea>
            ...
            <div>
                ...
                <select name="type" id="type" required class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
                    <option value="golf" {{ old('type', $subcategory->type) == 'golf' ? 'selected' : '' }}>Golf</option>
                    <option value="tennis" {{ old('type', $subcategory->type) == 'tennis' ? 'selected' : '' }}>Tennis</option>
                    <option value="fitness" {{ old('type', $subcategory->type) == 'fitness' ? 'selected' : '' }}>Sport & Fitness</option>
                    <option value="spa" {{ old('type', $subcategory->type) == 'spa' ? 'selected' : '' }}>Spa & Wellness</option>
                    <option value="other" {{ old('type', $subcategory->type) == 'other' ? 'selected' : '' }}>Autre</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $subcategory->display_order) }}" min="0"
                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
