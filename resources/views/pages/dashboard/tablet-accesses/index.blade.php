...
<div class="mb-6 flex items-center justify-between">
    <div>
        ...
        <p class="text-gray-600 dark:text-gray-400">Comptes « Client Chambre XXX » pour identifier chaque chambre sur la tablette</p>
    </div>
    <a href="{{ route('dashboard.tablet-accesses.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600 dark:bg-brand-600 dark:hover:bg-brand-700">
...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="GET" action="{{ route('dashboard.tablet-accesses.index') }}" class="flex flex-wrap gap-4">
        ...
        @if(request()->hasAny(['search']))
            <a href="{{ route('dashboard.tablet-accesses.index') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800">Réinitialiser</a>
        @endif
    </form>
</div>
...
<div class="rounded-lg border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="mt-2 text-gray-500 dark:text-gray-400">Créez un compte par chambre pour que la tablette identifie la chambre (connexion avec email / mot de passe).</p>
            <div class="mt-6">
                <a href="{{ route('dashboard.tablet-accesses.create') }}" class="inline-flex items-center px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600">Nouvel accès tablette</a>
            </div>
        </div>
    @endif
</div>
