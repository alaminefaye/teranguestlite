...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Mes Annonces</h1>
        <a href="{{ route('dashboard.enterprise-announcements.create') }}"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($announcements as $announcement)
            <div
                ...
                @else
                    <div
                        class="h-32 rounded-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center mb-3 text-gray-400 text-xs">
                        Pas de média</div>
                @endif

                @if(!$announcement->is_active)<span
                class="text-xs text-amber-600 dark:text-amber-400 font-medium">Inactive</span>@endif
                <h3 class="font-semibold text-gray-800 dark:text-white/90 truncate mb-1">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">👁 {{ number_format($announcement->view_count) }}
                    vue(s)</p>

                <div class="flex items-center gap-2">
                    <x-action-buttons :showRoute="route('dashboard.enterprise-announcements.show', $announcement)"
                        :editRoute="route('dashboard.enterprise-announcements.edit', $announcement)"
                        :deleteRoute="route('dashboard.enterprise-announcements.destroy', $announcement)"
                        deleteMessage="Supprimer cette annonce ?" />
                    <form action="{{ route('dashboard.enterprise-announcements.toggle', $announcement) }}" method="POST"
                        class="inline">
                        @csrf
                        <button type="submit"
        ...
        @empty
            <div class="col-span-3 text-center py-12">
                <p class="text-gray-600 dark:text-gray-400 mb-4">Aucune annonce. Créez-en une pour qu'elle apparaisse dans l'app
                    !</p>
                <a href="{{ route('dashboard.enterprise-announcements.create') }}"
