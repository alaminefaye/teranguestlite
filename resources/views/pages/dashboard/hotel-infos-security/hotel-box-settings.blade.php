...
<form action="{{ route('dashboard.hotel-box-settings.update') }}" method="POST" enctype="multipart/form-data">
    ...
    <div id="document-section"
         class="mb-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 {{ ($hotelBoxSettings['display_mode'] ?? 'catalog') === 'document' ? '' : 'hidden' }}">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                @if(!empty($hotelBoxSettings['document_url_fr']))
                    <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <a href="{{ $hotelBoxSettings['document_url_fr'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">{{ basename($hotelBoxSettings['document_path_fr'] ?? $hotelBoxSettings['document_url_fr']) }}</a>
                        </div>
                        <a href="{{ $hotelBoxSettings['document_url_fr'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
                    </div>
                @endif
            ...
            <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                @if(!empty($hotelBoxSettings['document_url_en']))
                    <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                        <span class="text-success-600 dark:text-success-400 text-xl">✅</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-success-700 dark:text-success-300">Version EN actuelle</p>
                            <a href="{{ $hotelBoxSettings['document_url_en'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">{{ basename($hotelBoxSettings['document_path_en'] ?? $hotelBoxSettings['document_url_en']) }}</a>
                        </div>
                        <a href="{{ $hotelBoxSettings['document_url_en'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
                    </div>
                @endif
        ...
        @if(!empty($hotelBoxSettings['legacy_document_url']))
            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                ...
                <a href="{{ $hotelBoxSettings['legacy_document_url'] }}" target="_blank" class="underline">{{ basename($hotelBoxSettings['legacy_document_path'] ?? $hotelBoxSettings['legacy_document_url']) }}</a>
            </div>
        @endif
