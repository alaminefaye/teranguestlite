...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
    <form method="GET" action="{{ route('dashboard.spa-reservations.index') }}" class="flex flex-wrap gap-4">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Service ou client..." class="rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 min-w-[200px]">
        <input type="date" name="date" value="{{ request('date') }}" class="rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90">
                ...
                @forelse($reservations as $r)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('dashboard.spa-reservations.show', $r) }}" class="text-brand-600 dark:text-brand-400 hover:underline text-sm font-medium">Voir</a>
                        </td>
                    </tr>
                @empty
