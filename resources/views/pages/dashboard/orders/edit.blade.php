...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
        ...
        <span>/</span>
        <a href="{{ route('dashboard.orders.show', $order) }}" class="hover:text-brand-500">{{ $order->order_number }}</a>
        <span>/</span>
        <span>Modifier</span>
    </div>
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Modifier commande #{{ $order->order_number }}</h1>
</div>
...
<div x-data="orderForm()" class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.orders.update', $order) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <!-- Type de commande (lecture seule) -->
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Type</label>
                <input type="text" value="{{ $order->type_name }}" disabled
                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90">
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Le type ne peut pas être modifié</p>
            </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="special_instructions" id="special_instructions" rows="3"
                class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">{{ old('special_instructions', $order->special_instructions) }}</textarea>
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('dashboard.orders.show', $order) }}" class="inline-flex items-center px-6 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800">
                Annuler
            </a>
        </div>
    </form>
...
<script>
    function orderForm() {
        return {
            items: @json($order->orderItems->map(function($item) {
                return [
            ...
            get deliveryFee() {
                return {{ $order->type === 'room_service' ? 1000 : 0 }};
            },
