...
<form action="{{ route('dashboard.sport-settings.update') }}" method="POST" enctype="multipart/form-data">
    ...
    <div id="document-section"
         ...
         style="{{ ($sportSettings['display_mode'] ?? 'catalog') === 'document' ? '' : 'display:none' }}">
        ...
        @if(!empty($sportSettings['document_url']))
            <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <a href="{{ $sportSettings['document_url'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">
                        {{ basename($sportSettings['document_path'] ?? $sportSettings['document_url']) }}
                    </a>
                </div>
                <a href="{{ $sportSettings['document_url'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
            </div>
        @endif
