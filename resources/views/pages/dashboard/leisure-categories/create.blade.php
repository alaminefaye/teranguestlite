...
<div class="mb-6">
    ...
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Ajouter une catégorie</h1>
</div>

<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.leisure-categories.store') }}" method="POST">
        @csrf
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="ex. Spa & Wellness"
            ...
            <div>
                <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description (affichée dans l’app)</label>
                <textarea name="description" id="description" rows="3" placeholder="ex. Consultation de la carte des soins et réservation instantanée des créneaux de massage."
            ...
            <div>
                ...
                <select name="type" id="type" required class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
                    ...
                    <option value="loisirs" {{ old('type') == 'loisirs' ? 'selected' : '' }}>Loisirs</option>
                </select>
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('dashboard.leisure-categories.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 text-sm">Annuler</a>
        </div>
    </form>
</div>
@endsection
