...
<div class="mb-6">
    ...
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Modifier la catégorie</h1>
</div>

<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.leisure-categories.update', $category) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <select name="type" id="type" required class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
                    <option value="sport" {{ old('type', $category->type) == 'sport' ? 'selected' : '' }}>Sport</option>
                    <option value="loisirs" {{ old('type', $category->type) == 'loisirs' ? 'selected' : '' }}>Loisirs</option>
                </select>
