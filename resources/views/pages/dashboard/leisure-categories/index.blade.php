...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="GET" action="{{ route('dashboard.leisure-categories.index') }}" class="flex flex-wrap gap-4 items-end">
        ...
        <div>
            ...
            <select name="type" class="rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 min-w-[140px]">
                <option value="">Tous</option>
                <option value="sport" {{ request('type') === 'sport' ? 'selected' : '' }}>Sport</option>
                <option value="loisirs" {{ request('type') === 'loisirs' ? 'selected' : '' }}>Loisirs</option>
            </select>
        </div>
        <button type="submit" class="px-4 py-2 bg-brand-500 text-white rounded-md hover:bg-brand-600">Filtrer</button>
        @if(request()->hasAny(['search', 'type']))
            <a href="{{ route('dashboard.leisure-categories.index') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800">Réinitialiser</a>
        @endif
    </form>
</div>
    ...
    @forelse($categories as $category)
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900 {{ !$category->is_active ? 'opacity-70' : '' }}">
            ...
            <div class="flex items-center gap-2 mt-3 flex-wrap">
                ...
                <x-action-buttons
                    :editRoute="route('dashboard.leisure-categories.edit', $category)"
                    :canDelete="false"
                />
