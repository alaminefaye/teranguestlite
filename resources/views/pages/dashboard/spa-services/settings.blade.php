...
<div class="mb-6 flex items-center gap-4">
    <a href="{{ route('dashboard.spa-services.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-sm flex items-center gap-1">
        ← Retour aux services
    </a>
</div>

<div class="mb-6">
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">⚙️ Paramètres d'affichage — Spa & Bien-être</h1>
    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Choisissez comment la section Spa s'affiche dans l'application mobile.</p>
</div>
...
@if(session('error'))
    <div class="mb-6 rounded-lg bg-red-50 p-4 text-red-600 dark:bg-red-500/10 dark:text-red-400">
        {{ session('error') }}
    </div>
@endif

<form action="{{ route('dashboard.spa-services.settings.update') }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mb-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label class="relative flex cursor-pointer rounded-xl border-2 p-5 transition-all
                {{ ($spaSettings['display_mode'] ?? 'catalog') === 'catalog'
                    ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/20'
                    : 'border-gray-200 dark:border-gray-700 hover:border-brand-300' }}">
                <input type="radio" name="display_mode" value="catalog" class="sr-only"
                    {{ ($spaSettings['display_mode'] ?? 'catalog') === 'catalog' ? 'checked' : '' }}
                    onchange="this.form.querySelector('#document-section').style.display='none'">
                <div class="flex items-start gap-4">
                    <div class="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-brand-100 dark:bg-brand-900/40 text-2xl">
                        🗂️
                    </div>
                    <div>
                        <p class="font-semibold text-gray-800 dark:text-white/90">Catalogue de services</p>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Affiche la grille des services spa (Hammam, Massage, Manucure…) avec réservation.</p>
                    </div>
                </div>
            </label>
            ...
            <!-- Option : Document -->
            <label class="relative flex cursor-pointer rounded-xl border-2 p-5 transition-all
                {{ ($spaSettings['display_mode'] ?? 'catalog') === 'document'
                    ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/20'
                    : 'border-gray-200 dark:border-gray-700 hover:border-brand-300' }}">
                <input type="radio" name="display_mode" value="document" class="sr-only"
                    {{ ($spaSettings['display_mode'] ?? 'catalog') === 'document' ? 'checked' : '' }}
                    onchange="this.form.querySelector('#document-section').style.display='block'">
                <div class="flex items-start gap-4">
                    <div class="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-amber-100 dark:bg-amber-900/40 text-2xl">
                        📄
                    </div>
                    <div>
                        <p class="font-semibold text-gray-800 dark:text-white/90">Document (PDF / Image)</p>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Ouvre directement un document dans l'app (menu PDF, brochure, tarifs…).</p>
                    </div>
                </div>
            </label>
        </div>
    ...
    <!-- Section upload document (visible si mode = document) -->
    <div id="document-section"
         class="mb-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900 {{ ($spaSettings['display_mode'] ?? 'catalog') === 'document' ? '' : 'hidden' }}">
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                @if(!empty($spaSettings['document_url_fr']))
                    <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <a href="{{ $spaSettings['document_url_fr'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">
                                {{ basename($spaSettings['document_path_fr'] ?? $spaSettings['document_url_fr']) }}
                            </a>
                        </div>
                        <a href="{{ $spaSettings['document_url_fr'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
                    </div>
                @endif
                <label for="document-upload-fr" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-pointer bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p class="text-xs text-gray-400 dark:text-gray-500">PDF, JPG, PNG, WEBP</p>
                    </div>
            ...
            <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                ...
                @if(!empty($spaSettings['document_url_en']))
                    <div class="mb-4 flex items-center gap-3 rounded-lg border border-success-200 bg-success-50 p-3 dark:border-success-800 dark:bg-success-900/20">
                        <span class="text-success-600 dark:text-success-400 text-xl">✅</span>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-success-700 dark:text-success-300">Version EN actuelle</p>
                            <a href="{{ $spaSettings['document_url_en'] }}" target="_blank" class="text-xs text-success-600 dark:text-success-400 hover:underline truncate block">
                                {{ basename($spaSettings['document_path_en'] ?? $spaSettings['document_url_en']) }}
                            </a>
                        </div>
                        <a href="{{ $spaSettings['document_url_en'] }}" target="_blank" class="text-xs text-brand-600 dark:text-brand-400 hover:underline shrink-0">Voir ↗</a>
                    </div>
                @endif
        ...
        @if(!empty($spaSettings['legacy_document_url']))
            <div class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                ...
                <a href="{{ $spaSettings['legacy_document_url'] }}" target="_blank" class="underline">{{ basename($spaSettings['legacy_document_path'] ?? $spaSettings['legacy_document_url']) }}</a>
            </div>
        @endif
    ...
    <div class="flex justify-end gap-3">
        <a href="{{ route('dashboard.spa-services.index') }}"
           class="px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">
            Annuler
        </a>
        <button type="submit"
                class="px-5 py-2.5 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600 dark:bg-brand-600 dark:hover:bg-brand-700">
...
<script>
    // Highlight selected card when radio changes
