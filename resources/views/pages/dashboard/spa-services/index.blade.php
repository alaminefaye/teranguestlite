...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="GET" action="{{ route('dashboard.spa-services.index') }}" class="space-y-4">
        <div class="flex flex-wrap gap-4 items-end">
            ...
            <div>
                ...
                <select name="category" class="rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 min-w-[160px]">
                    ...
                    <option value="facial" {{ request('category') === 'facial' ? 'selected' : '' }}>Soin du visage</option>
                    <option value="body_treatment" {{ request('category') === 'body_treatment' ? 'selected' : '' }}>Soin du corps</option>
                    <option value="wellness" {{ request('category') === 'wellness' ? 'selected' : '' }}>Bien-être</option>
                </select>
            </div>
            <div>
            ...
            @if(request()->hasAny(['search', 'category', 'status']))
                <a href="{{ route('dashboard.spa-services.index') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800">Réinitialiser</a>
            @endif
        </div>
    </form>
</div>
