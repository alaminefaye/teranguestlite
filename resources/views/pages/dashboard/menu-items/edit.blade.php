            ...
            <div class="md:col-span-2 rounded-lg border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-800/50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="stock_product_id" id="stock_product_id" class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90">
                            ...
                            @foreach($stockProducts ?? [] as $sp)
                                <option value="{{ $sp->id }}" {{ old('stock_product_id', $item->stock_product_id) == $sp->id ? 'selected' : '' }}>{{ $sp->name }} ({{ $sp->category?->name ?? '' }}) — {{ number_format($sp->quantity_current, 0, ',', ' ') }} {{ $sp->unit_label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="stock_quantity_per_portion" id="stock_quantity_per_portion" value="{{ old('stock_quantity_per_portion', $item->stock_quantity_per_portion ?? 1) }}" min="0.001" step="any" class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90" placeholder="1">
