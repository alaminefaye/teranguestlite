...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
        ...
        <span>/</span>
        <a href="{{ route('dashboard.laundry-services.show', $service) }}" class="hover:text-brand-500">{{ $service->name }}</a>
        <span>/</span>
        <span>Modifier</span>
    </div>
...
<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.laundry-services.update', $service) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="category" id="category" required class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
                    <option value="washing" {{ old('category', $service->category) == 'washing' ? 'selected' : '' }}>Lavage</option>
                    <option value="ironing" {{ old('category', $service->category) == 'ironing' ? 'selected' : '' }}>Repassage</option>
                    <option value="dry_cleaning" {{ old('category', $service->category) == 'dry_cleaning' ? 'selected' : '' }}>Nettoyage à sec</option>
                    <option value="express" {{ old('category', $service->category) == 'express' ? 'selected' : '' }}>Express</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="turnaround_hours" id="turnaround_hours" value="{{ old('turnaround_hours', $service->turnaround_hours) }}" required min="1"
                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
        ...
        <div class="mt-6 flex items-center gap-3">
            ...
            <a href="{{ route('dashboard.laundry-services.show', $service) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 text-sm">Annuler</a>
        </div>
    </form>
</div>
@endsection
