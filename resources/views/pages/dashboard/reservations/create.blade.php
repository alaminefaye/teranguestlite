...
<div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
    <form action="{{ route('dashboard.reservations.store') }}" method="POST" x-data="{
        checkIn: '{{ old('check_in', date('Y-m-d\TH:i')) }}',
        checkOut: '{{ old('check_out', date('Y-m-d\TH:i', strtotime('+1 day'))) }}',
        roomId: '{{ old('room_id') }}',
        pricePerNight: 0,
        calculateTotal() {
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <select name="guest_id" id="guest_id" required
                    ...
                    @foreach($guests as $g)
                        <option value="{{ $g->id }}" {{ old('guest_id') == $g->id ? 'selected' : '' }}>
            ...
            <!-- Compte client (optionnel) : lie la résa au compte app pour les notifications push -->
            <div class="md:col-span-2">
                ...
                <select name="user_id" id="user_id"
                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 py-2 text-gray-800 dark:text-white/90 focus:border-brand-500 focus:ring-brand-500">
                    <option value="">Aucun (notifications via tablette / code uniquement)</option>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Si le client a un compte app, sélectionnez-le pour que les notifications (commandes, réservations) arrivent sur son téléphone.</p>
                @error('user_id')
                    <p class="mt-1 text-sm text-error-600 dark:text-error-400">{{ $message }}</p>
                @enderror
            </div>
