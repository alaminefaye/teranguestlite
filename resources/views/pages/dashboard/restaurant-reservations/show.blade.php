...
@section('content')
<div class="mb-6">
    <a href="{{ route('dashboard.restaurant-reservations.index') }}" class="text-brand-600 dark:text-brand-400 text-sm mb-2 inline-block">← Retour aux réservations restaurants</a>
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        ...
        <dl class="space-y-3 text-sm">
            ...
            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Convives</dt><dd class="font-medium">{{ $reservation->number_of_guests ?? '—' }}</dd></div>
        ...
        <div class="mt-6 flex flex-wrap gap-2">
            @if($reservation->status !== 'cancelled')
                <a href="{{ route('dashboard.restaurant-reservations.edit', $reservation) }}" class="inline-flex items-center px-4 py-2 rounded-md bg-brand-500 text-white hover:bg-brand-600">Modifier</a>
                <form action="{{ route('dashboard.restaurant-reservations.cancel', $reservation) }}" method="POST" class="inline" onsubmit="return confirm('Confirmer l\'annulation de cette réservation ?')">
                    @csrf
