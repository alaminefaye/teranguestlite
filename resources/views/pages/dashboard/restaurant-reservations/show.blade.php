...
@section('content')
<div class="mb-6">
    <a href="{{ route('dashboard.restaurant-reservations.index') }}" class="text-brand-600 dark:text-brand-400 text-sm mb-2 inline-block">← Retour aux réservations restaurant</a>
    <h1 class="text-title-md2 font-semibold text-gray-900 dark:text-white/90">Réservation restaurant</h1>
</div>
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        ...
        <dl class="space-y-3 text-sm">
            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Restaurant</dt><dd class="font-medium">{{ $reservation->restaurant?->name ?? '—' }}</dd></div>
            ...
            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Nombre de convives</dt><dd>{{ $reservation->number_of_guests ?? '—' }}</dd></div>
            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Statut</dt>
                <dd><span class="inline-flex rounded-full px-2 py-1 text-xs font-medium @if($reservation->status === 'confirmed') bg-success-50 text-success-600 dark:bg-success-500/10 dark:text-success-400 @elseif($reservation->status === 'cancelled') bg-error-50 text-error-600 dark:bg-error-500/10 dark:text-error-400 @else bg-warning-50 text-warning-600 dark:bg-warning-500/10 dark:text-warning-400 @endif">{{ $reservation->status }}</span></dd>
            </div>
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        ...
        @else
        <p class="text-gray-500 dark:text-gray-400 text-sm">@if($reservation->room)Client Chambre {{ $reservation->room->room_number }}@else—@endif</p>
        @endif
    </div>
</div>
@endsection
