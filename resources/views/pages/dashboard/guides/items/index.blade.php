...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('dashboard.guide-items.create', ['category' => $category?->id]) }}"
            ...
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-800/50">
                    <tr class="border-b border-gray-200 dark:border-gray-800">
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Titre</th>
                        @if(!$category)
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Catégorie</th>
                        @endif
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-400 uppercase">Contact / Adresse</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                    @forelse($items as $item)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            @endif
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">
                                @if($item->phone)<div>📞 {{ $item->phone }}</div>@endif
                                @if($item->address)<div>📍 {{ $item->address }}</div>@endif
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $item->order }}</td>
                            <td class="px-6 py-4">
                                <form action="{{ route('dashboard.guide-items.toggle', $item) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <x-action-buttons
                                        :editRoute="route('dashboard.guide-items.edit', $item)"
                                        :deleteRoute="route('dashboard.guide-items.destroy', $item)"
