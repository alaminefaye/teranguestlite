...
@section('content')
    ...
    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        <form action="{{ route('dashboard.guide-categories.update', $category) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category_type"
                        ...
                        <option value="">—</option>
                        <option value="equipment_guide" {{ old('category_type', $category->category_type) === 'equipment_guide' ? 'selected' : '' }}>Guide équipements</option>
                        <option value="useful_numbers" {{ old('category_type', $category->category_type) === 'useful_numbers' ? 'selected' : '' }}>Numéros utiles</option>
                        <option value="other" {{ old('category_type', $category->category_type) === 'other' ? 'selected' : '' }}>Autre</option>
                    </select>
                    @error('category_type')
                        <p class="mt-1 text-sm text-error-600 dark:text-error-400">{{ $message }}</p>
                    @enderror
