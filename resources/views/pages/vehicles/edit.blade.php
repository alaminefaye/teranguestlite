...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-title-md font-semibold text-gray-800 dark:text-white/90">Modifier le Véhicule</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Modifiez les informations du véhicule</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="navette" {{ old('type', $vehicle->type) == 'navette' ? 'selected' : '' }}>Navette</option>
                ...
                <div>
                    ...
                    <input type="number" name="capacity" value="{{ old('capacity', $vehicle->capacity) }}" min="1" required
                           class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    @error('capacity')
                        <p class="mt-1 text-xs text-error-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="status" required
                        ...
                        <option value="in_use" {{ old('status', $vehicle->status) == 'in_use' ? 'selected' : '' }}>En cours</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-xs text-error-500">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="chauffeur_id"
                        ...
                        @foreach($chauffeurs as $chauffeur)
                            <option value="{{ $chauffeur->id }}" {{ old('chauffeur_id', $vehicle->chauffeur_id) == $chauffeur->id ? 'selected' : '' }}>
                                {{ $chauffeur->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    @if($vehicle->photo)
                        <div class="mb-2">
                            <img src="{{ Storage::url($vehicle->photo) }}" alt="{{ $vehicle->name }}" class="h-20 w-20 rounded-lg object-cover">
                        </div>
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">{{ old('description', $vehicle->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-error-500">{{ $message }}</p>
                    @enderror
                </div>
