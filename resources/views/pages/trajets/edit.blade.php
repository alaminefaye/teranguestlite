...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-title-md font-semibold text-gray-800 dark:text-white/90">Modifier le Trajet</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Modifiez les informations du trajet</p>
        </div>
    </div>

    <div class="rounded-lg border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-gray-900">
        <form action="{{ route('trajets.update', $route) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    <select name="departure_station_id" required
                        ...
                        @foreach($stations as $station)
                            <option value="{{ $station->id }}" {{ old('departure_station_id', $route->departure_station_id) == $station->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="arrival_station_id" required
                            class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                        @foreach($stations as $station)
                            <option value="{{ $station->id }}" {{ old('arrival_station_id', $route->arrival_station_id) == $station->id ? 'selected' : '' }}>
                                {{ $station->name }} - {{ $station->city }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="distance" value="{{ old('distance', $route->distance) }}" step="0.01" min="0"
                ...
                <div>
                    ...
                    <input type="number" name="duration" value="{{ old('duration', $route->duration) }}" min="0"
                           class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    @error('duration')
                        <p class="mt-1 text-xs text-error-500">{{ $message }}</p>
                    @enderror
