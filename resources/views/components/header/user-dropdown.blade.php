<div class="relative" x-data="{
    ...
    >
        <!-- User Info -->
        <div>
            <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ Auth::check() ? Auth::user()->name : 'Invité' }}</span>
            <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">{{ Auth::check() ? Auth::user()->email : 'Non connecté' }}</span>
            @if(Auth::check() && Auth::user()->role)
                <span class="mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium bg-brand-50 text-brand-600 dark:bg-brand-500/10 dark:text-brand-400">
