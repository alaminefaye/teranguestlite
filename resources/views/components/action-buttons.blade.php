@props(['showRoute' => null, 'editRoute' => null, 'deleteRoute' => null, 'cancelRoute' => null, 'invoiceRoute' => null, 'deleteMessage' => 'Êtes-vous sûr de vouloir supprimer cet élément ?', 'canDelete' => true, 'canCancel' => true])
...
<div class="flex items-center justify-end gap-2">
    ...
    @if($invoiceRoute)
        <a href="{{ $invoiceRoute }}"
           ...
           title="Voir la facture / Imprimer">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($cancelRoute && $canCancel)
        <form action="{{ $cancelRoute }}" method="POST" class="inline" onsubmit="return confirm('Confirmer l\'annulation ?')">
            @csrf
            @method('POST')
            <button type="submit"
                    class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-warning-50 text-warning-600 hover:bg-warning-100 dark:bg-warning-500/10 dark:text-warning-400 dark:hover:bg-warning-500/20 transition-colors"
                    title="Annuler">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
